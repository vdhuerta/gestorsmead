        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            {['individual', 'institutional', 'search'].map((t) => (
                <button key={t} onClick={() => setActiveTab(t as TMSTab)} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === t ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{t === 'individual' ? 'Perfil' : t === 'institutional' ? 'Cobertura' : 'Buscador'}</button>
            ))}
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col overflow-hidden border border-indigo-100">
            ...
            <div className="flex-1 overflow-y-auto p-10 bg-[#F9F8F6] custom-scrollbar space-y-12">
                <section className="space-y-4">
                    <div className="flex items-center gap-3 border-b-2 border-indigo-100 pb-2"><span className="w-8 h-8 bg-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center font-bold text-sm">01</span><h4 className="text-xl font-black text-slate-800 uppercase tracking-tight">Módulo "Perfil"</h4></div>
