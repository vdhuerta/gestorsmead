          ...
          <div className="animate-fadeIn max-w-4xl mx-auto">
              ...
              <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-8">
                  ...
                  <form onSubmit={handleCreateSubmit} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                              ...
                              <div>
                                  ...
                                  {view === 'create' && <p className="text-[10px] text-indigo-600 mt-1 font-bold italic">Autogenerado 6 semanas</p>}
                              </div>
                              <div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <div><label className="block text-sm font-medium text-slate-700 mb-1">Código Interno</label><input type="text" value={formData.internalCode} onChange={e => setFormData({...formData, internalCode: e.target.value})} className="w-full px-4 py-2 border border-slate-300 rounded-lg font-mono text-xs bg-slate-50"/>{isAdvisor && <p className="text-[10px] text-green-600 mt-1 font-bold">Código Identificador</p>}</div>
                      ...
                      <div className="pt-6 border-t border-slate-100 flex justify-end"><button type="submit" disabled={isSyncing} className={`bg-[#647FBC] hover:bg-blue-800 text-white px-8 py-3 rounded-lg font-bold shadow-lg transition-all flex items-center gap-2 ${isSyncing ? 'opacity-70 cursor-wait' : 'active:scale-95'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{view === 'create' ? 'Crear Curso' : 'Guardar Cambios'}</button></div>
                  </form>
              </div>
          </div>);
  }
