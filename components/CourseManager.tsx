  ...
  const handleBulkUpload = () => {
      ...
      reader.onload = async (e) => {
          ...
          for (let i = startRow; i < rows.length; i++) {
              ...
              // --- LOGICA DE RESCATE DE DATOS ---
              const masterUser = users.find(u => normalizeRut(u.rut) === normRut);
              
              // Solo agregamos a usersToUpsert si la fila trae datos o si el usuario no existe
              // Si la fila trae nombre, priorizamos el dato del Excel pero respetamos datos de la Base Maestra si el excel viene vacio
              const hasNameInRow = rowStrings[1] && rowStrings[1].length > 1;

              if (hasNameInRow || !masterUser) {
                  // Mapeo inteligente: Si el excel no trae el campo, usamos el de la base maestra si existe
                  usersToUpsert.push({
                      rut: cleanRut,
