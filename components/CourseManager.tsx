...
export const CourseManager: React.FC<CourseManagerProps> = ({ currentUser }) => {
  ...
  const handleUpdateGrade = async (enrollmentId: string, gradeIndex: number, value: string) => {
      ...
      await updateEnrollment(enrollmentId, { grades: currentGrades, finalGrade: finalGrade, state: newState });
      await executeReload();
  };
  ...
  const handleToggleAttendance = async (enrollmentId: string, sessionIndex: number) => {
      ...
      await updateEnrollment(enrollmentId, { [fieldName]: newVal, attendancePercentage: percentage, state: newState });
      await executeReload();
  };
  ...
  const handleBulkUpload = () => {
      ...
      reader.onload = async (e) => {
          ...
          // --- FIX: LOCAL UNIQUE TRACKING TO PREVENT POSTGRES ERROR ---
          const processedRuts = new Set<string>(); // Para evitar duplicados dentro del mismo archivo
          ...
          for (let i = startRow; i < rows.length; i++) {
              ...
              // SI YA PROCESAMOS ESTE RUT EN ESTA CARGA, SALTARLO (Previene error ON CONFLICT)
          ...
          if (rutsToEnroll.length > 0) {
              try {
                  ...
                  await executeReload();
                  setEnrollMsg({ type: 'success', text: `Carga Masiva: ${result.success} nuevos inscritos, ${rows.length - startRow - rutsToEnroll.length} omitidos.` });
              } catch (enrollErr: any) {
                  console.error("Bulk enrollment error:", enrollErr);
                  setEnrollMsg({ type: 'error', text: `Error al matricular: ${enrollErr.message}` });
              }
          } else {
              setEnrollMsg({ type: 'success', text: 'Proceso finalizado: No había nuevos RUTs para matricular.' });
          }
          setUploadFile(null);
      };
